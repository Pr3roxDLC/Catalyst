...
public class CrystalUtils {
    ...
    public static boolean rayTraceSolidCheck(Vec3d start, Vec3d end, boolean shouldIgnore) {
        if (!Double.isNaN(start.x) && !Double.isNaN(start.y) && !Double.isNaN(start.z)) {
            if (!Double.isNaN(end.x) && !Double.isNaN(end.y) && !Double.isNaN(end.z)) {
                ...
                if ((blockState.getCollisionBoundingBox(Minecraft.getMinecraft().world, blockPos) != Block.NULL_AABB) &&
                ...
                while (steps-- >= 0) {
                    ...
                    blockState = Minecraft.getMinecraft().world.getBlockState(blockPos);
    ...
    public static float getDamageFromDifficulty(float damage) {
        switch (Minecraft.getMinecraft().world.getDifficulty()) {
    ...
    public static float getExplosionDamage(Entity targetEntity, Vec3d explosionPosition, float explosionPower, boolean shouldIgnore) {
        ...
        if (targetEntity instanceof EntityLivingBase)
            damage = getBlastReduction((EntityLivingBase) targetEntity, CrystalUtils.getDamageFromDifficulty(damage),
                    new Explosion(Minecraft.getMinecraft().world, null, explosionPosition.x, explosionPosition.y, explosionPosition.z,
    ...
    public static List<BlockPos> possiblePlacePositions(float placeRange, boolean specialEntityCheck, boolean oneDot15) {
        ...
        positions.addAll(getSphere(EntityUtils.getPlayerPos(), placeRange, (int) placeRange, false, true, 0).stream().filter(pos -> Minecraft.getMinecraft().world.getBlockState(pos).getBlock() != Blocks.AIR).filter(pos -> canPlaceCrystal(pos, specialEntityCheck, oneDot15)).collect(Collectors.toList()));
        return positions;
    }
    ...
    public static boolean canSeePos(BlockPos pos) {
        return Minecraft.getMinecraft().world.rayTraceBlocks(new Vec3d(Minecraft.getMinecraft().player.posX, Minecraft.getMinecraft().player.posY + (double) Minecraft.getMinecraft().player.getEyeHeight(), Minecraft.getMinecraft().player.posZ), new Vec3d(pos.getX(), pos.getY(), pos.getZ()), false, true, false) == null;
    }
    ...
    public static boolean canPlaceCrystal(BlockPos blockPos, boolean specialEntityCheck, boolean onepointThirteen) {
        ...
        try {
            if (!onepointThirteen) {
                ...
                if (Minecraft.getMinecraft().world.getBlockState(boost).getBlock() != Blocks.AIR || Minecraft.getMinecraft().world.getBlockState(boost2).getBlock() != Blocks.AIR) {
                    return false;
                }
                if (!specialEntityCheck) {
                    return Minecraft.getMinecraft().world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(boost)).isEmpty() && Minecraft.getMinecraft().world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(boost2)).isEmpty();
                }
                ...
                if (!specialEntityCheck) {
                    return Minecraft.getMinecraft().world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(boost)).isEmpty();
                }
                for (Entity entity : Minecraft.getMinecraft().world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(boost))) {
                    if (entity instanceof EntityEnderCrystal) continue;
                    return false;
